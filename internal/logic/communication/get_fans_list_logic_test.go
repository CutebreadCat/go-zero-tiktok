package communication

import (
	"context"
	"errors"
	"testing"

	"go_zero-tiktok/internal/svc/mock"
	"go_zero-tiktok/internal/types"
)

func TestGetFansListLogic_GetFansList(t *testing.T) {
	tests := []struct {
		name       string
		ctx        context.Context
		req        *types.GetFansListRequest
		userFollow *mock.UserFollowRepo
		userRepo   *mock.UserRepo
		wantErr    bool
		wantLen    int
	}{
		{
			name: "get fans failed",
			ctx:  context.WithValue(context.Background(), "user_id", "u1"),
			req:  &types.GetFansListRequest{},
			userFollow: &mock.UserFollowRepo{
				GetFansByUserIDFn: func(ctx context.Context, userID string, pageNumber, pageSize int32) ([]types.UserFollow, int64, error) {
					return nil, 0, errors.New("db error")
				},
			},
			wantErr: true,
		},
		{
			name: "success",
			ctx:  context.WithValue(context.Background(), "user_id", "u1"),
			req:  &types.GetFansListRequest{PageNumber: 1, PageSize: 10},
			userFollow: &mock.UserFollowRepo{
				GetFansByUserIDFn: func(ctx context.Context, userID string, pageNumber, pageSize int32) ([]types.UserFollow, int64, error) {
					return []types.UserFollow{
						{FollowerID: "u2", UserID: "u1"},
						{FollowerID: "u3", UserID: "u1"},
						{FollowerID: "u4", UserID: "u1"},
					}, 3, nil
				},
			},
			userRepo: &mock.UserRepo{
				GetUsersByIDsFn: func(ctx context.Context, userIDs []string) ([]types.UserBaseinfo, error) {
					return []types.UserBaseinfo{
						{UserID: "u2", Username: "fan1"},
						{UserID: "u3", Username: "fan2"},
						{UserID: "u4", Username: "fan3"},
					}, nil
				},
			},
			wantErr: false,
			wantLen: 3,
		},
		{
			name: "success empty",
			ctx:  context.WithValue(context.Background(), "user_id", "u1"),
			req:  &types.GetFansListRequest{},
			userFollow: &mock.UserFollowRepo{
				GetFansByUserIDFn: func(ctx context.Context, userID string, pageNumber, pageSize int32) ([]types.UserFollow, int64, error) {
					return nil, 0, nil
				},
			},
			userRepo: &mock.UserRepo{
				GetUsersByIDsFn: func(ctx context.Context, userIDs []string) ([]types.UserBaseinfo, error) {
					return nil, nil
				},
			},
			wantErr: false,
			wantLen: 0,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svcCtx := mock.NewServiceContext(tt.userRepo, nil, nil, nil, nil, tt.userFollow, nil)
			logic := NewGetFansListLogic(tt.ctx, svcCtx)
			resp, err := logic.GetFansList(tt.req)
			if tt.wantErr {
				if err == nil {
					t.Errorf("expected error, got nil")
				}
				return
			}
			if err != nil {
				t.Errorf("unexpected error: %v", err)
				return
			}
			if len(resp.FansList) != tt.wantLen {
				t.Errorf("expected %d fans, got %d", tt.wantLen, len(resp.FansList))
			}
		})
	}
}
